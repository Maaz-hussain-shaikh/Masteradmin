...
type EditcardmodalProps = {
  cardData: CardProps;   // yaha type banaya
};

const Editcardmodal: React.FC<EditcardmodalProps> = ({ cardData }) => {
    const token = localStorage.getItem("token");
    const user = localStorage.getItem("username");
    const { slug } = useParams();

    const [editdata, setEdit] = useState({
        tripname: cardData.name_of_trip,
        tripduration: cardData.duration,
        offer: cardData.discount,
        mainprice: cardData.price_of_trip,
        tag: cardData.cards_Tag,
        tag2: cardData.cards_Tag2,
        type: cardData.type_of_trip,
        country: cardData.country,
        state: cardData.state,
        city: "",
        isSpecialevent: cardData.is_event,
        seasonoftrip: cardData.seasonoftrip,
        nameofevent: cardData.Nameofevent,
        status: false,
    ...
    const handleSubmit = async (e: any) => {
        ...
        try {
            ...
            form.append("discount", editdata.offer.toString());
            form.append("type_of_trip", editdata.type);
            form.append("country", editdata.country);
            form.append("state", editdata.state.toString());
            ...
            console.log(form)
            const responce = await axios.put(API_URLS.itinerary.Editcard(slug,cardData.itinerary_cards_id),
